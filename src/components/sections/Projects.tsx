    ...
    <div ref={sectionRef} className="md:hidden bg-cream pb-12">
      ...
      <div className="flex flex-col gap-4 px-4">
        {projects.map((project, index) => {
          const imageSrc = project.hoverImage || project.images[0];
          return (
          <Link
            key={project.id}
            ...
            <div className="p-3 pb-0">
              ...
              >
                {imageSrc ? (
                  ...
                  <div className="mc-img absolute inset-0 flex items-center justify-center bg-[#1b1b19] text-center" aria-label={`${project.title} placeholder`}>
                    <span className="px-6 font-mono text-xs uppercase tracking-[0.2em] text-white/45">MARPROJECTOR / {String(index + 1).padStart(2, '0')}</span>
                  </div>
                )}
                <div
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    getAllProjects().slice(0, 4).forEach((project) => {
      const imageUrl = project.hoverImage || project.images[0];
      if (!imageUrl) return;
                ...
                return (
                  ...
                  >
                    {imgUrl ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center bg-[#1b1b19] text-center">
                        <span className="font-mono text-xs uppercase tracking-[0.2em] text-white/45">MARPROJECTOR / {String(project.id).padStart(2, '0')}</span>
                      </div>
                    )}
                  </div>
                );
